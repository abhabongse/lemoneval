...
from lemoneval.core.testcase import SimpleTest, TestOperation
...
class DefaultTestConfiguration(SimpleTestConfiguration):
    ...
    test_structure = TestOperation.sum(
        TestOperation.max(
            TestOperation.min(test_cases[0], test_cases[1]),
            TestOperation.min(test_cases[2], test_cases[3])
            ),
        20
        )
    score_limit = 40
